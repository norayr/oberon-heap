...
BEGIN
   ...
   FOR k := 0 TO PIECE_ORIENTATIONS - 1 DO
      FOR i := 0 TO PIECE_SIZE - 1 DO p.cache[k][i] := NEW(PieceCell); 
END;
...
VAR i : LONGINT;
BEGIN
   FOR i := 0 TO PIECE_SIZE - 1 DO p.cache[p.orientation][i].Unmark(); 
END;
END Unmark;
...
VAR i : LONGINT;
BEGIN FOR i := 0 TO BOARD_SIZE - 1 DO b.cells[i].Unmark(); END; END 
Unmark;
...
BEGIN
   ...
   IF a = NIL THEN
      ...
      IF b.cellCount # PIECE_SIZE THEN
         b.cache[p.number][p.orientation][pieceIndex][boardIndex] := 
noFit;
         RETURN FALSE;
      END;

      NEW(a);
      FOR i := 0 TO PIECE_SIZE - 1 DO a[i] := b.cellsPieceWillFill[i]; 
END;
...
VAR
   ...
   BEGIN FOR i := 0 TO BOARD_SIZE - 1 DO a[i] := current[i]; END; END 
CopyTo;
BEGIN
   IF once THEN
      FOR i := 0 TO BOARD_SIZE - 1 DO
...
BEGIN
   board.Unmark();
   FOR i := 0 TO BOARD_SIZE - 1 DO
      forall := (board.cells[i].ContiguousEmptyCells() MOD PIECE_SIZE) = 
0;
